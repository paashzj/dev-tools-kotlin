...
import module.TroubleMysqlEnum
import widget.component.RowPaddingButton

val idx = mutableStateOf(TroubleMysqlEnum.SLOW_SQL)

val editMysqlInstanceName = mutableStateOf("")
...
@Composable
fun Head(idx: MutableState<TroubleMysqlEnum>) {
    Row {
        Text(R.strings.sceneList, fontSize = 40.sp)
        RowPaddingButton(
            onClick = {
                idx.value = TroubleMysqlEnum.SLOW_SQL
            },
        ) {
            Text(text = R.strings.TroubleSceneMysqlSlowSql)
        }
    }
}
