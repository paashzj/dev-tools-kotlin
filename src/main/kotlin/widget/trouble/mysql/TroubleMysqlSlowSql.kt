...
@Composable
fun TroubleMysqlSlowSql() {
    TroubleShootBase(
        content = {
            val mysqlNameList = StorageMysql.getInstance().listConfigNames()
            DropdownList(mysqlNameList, "mysql ${R.strings.instance}", editMysqlInstanceName)
            ...
            if (editMysqlInstanceName.value != "") {
                val mysqlConfig = StorageMysql.getInstance().getConfig(editMysqlInstanceName.value)
                editMysqlNamespace.value = mysqlConfig.namespace
                editMysqlStatefulSetName.value = mysqlConfig.statefulSetName
                val kubernetesConfig = StorageK8s.getInstance().getConfig(mysqlConfig.k8sName)
